...
class UserController
{
    ...
    {
        if (
            $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["name"]) && isset($_POST["lastName"]) &&
    ...
    {
        if (
            $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"]) && isset($_POST["name"]) && isset($_POST["lastName"]) &&
            isset($_POST["username"]) && isset($_POST["email"]) && isset($_POST["telephone"]) &&
            isset($_POST["password"]) && isset($_POST["role"])
        ) {
            ...
            if (is_numeric($id)) {
                ...
                try {
                    if ($this->userRepository->delete($id)) {
                        ...
                        echo "No se pudo eliminar el usuario. Verifique el ID proporcionado.";
                    }
                } catch (Exception $exception) {
                    echo "Error al eliminar el usuario :C" . $exception->getMessage();
                }
            }else{
                echo "no se ha ingresado un id correcto";
            }
        } else {
            echo "Faltan datos en el formulario >:(";
        }
